...
function connectMqtt() {
  global $aaa;
  $mcg = &$aaa['mqtt'];
  $client = $mc['client'] = new MqttClient($mcg['server'], $mcg['port'], $mcg['mqttClientId'], $mcg['mqtt_version']);
...
function setConfig($payloadIn) {
  ...
  $client->subscribe($aaa["topics"]["subscribe"]["cmd"], function ($_topic, $_payloadIn) {
    processCmd($_topic, $_payloadIn);
  });
//$client->subscribe($aaa["topics"]["subscribe"]["req"], function ($_topic, $_payloadIn) {
//  processReq($_topic, $_payloadIn);
//});
