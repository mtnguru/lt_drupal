...
((Drupal, settings, once) => {
  const colorSchemeOptions = settings.olive.colorSchemes;
  ...
  function synchronizeInputs(changedInput, inputToSync) {
    ...
    changedInput.setAttribute('data-olive-custom-color', changedInput.value);
    inputToSync.setAttribute('data-olive-custom-color', changedInput.value);
  ...
  function setColorScheme({ target }) {
    ...
    if (selectedColorScheme) {
      ...
      document
        .querySelectorAll(`input[data-olive-custom-color]`)
        .forEach((input) => {
          input.value = input.getAttribute('data-olive-custom-color');
        });
    }
  }
  ...
  function initColorPicker(textInput) {
    ...
    colorInput.setAttribute(
      'data-olive-custom-color',
      textInput.getAttribute('data-olive-custom-color'),
    );
   ...
   */
  Drupal.behaviors.oliveColorPicker = {
    attach: () => {
      ...
      const colorTextInputs = once(
        'olive-color-picker',
        '[data-drupal-selector="olive-color-picker"] input[type="text"]',
      );
